...
| **Last verified against commit** | `67c9de7e1` (full trace); water rows W.2a + W.2b.2 + W.2c.2 weave-deformation + W.2c.3 weave producer (2026-06-22) + F.4.2 weave-impact accent machinery (2026-06-24) |
...
| `astraweave-render/src/water.rs` | Chunked-LOD ocean surface (Gerstner waves) + refraction + weave deformation | Active | ~1020 LoC. **W.2a**: camera-distance discrete chunk grid, per-LOD instanced tiles + crack-hiding skirts; world Y from a `water_level` uniform (no baked mesh Y); `set_water_level` live (incl. editor knob). **W.2b.2**: water is now a SEPARATE post-opaque raster pass (`Renderer::run_water_pass`), no longer inside the shared main pass — it samples a scene-color snapshot for screen-space refraction and read-only scene depth for depth-delta shoreline foam; bind group is 4 entries (uniform + scene_color + scene_depth + sampler); `prepare_scene` wires the snapshot/depth each frame. **W.2c.2**: `WaterRenderer` now owns a **weave-deformation instance list** (`WeaveInstance`/`WeaveKind`, ceiling `MAX_WEAVE_INSTANCES = 8`) via `set_weave_instances`/`clear_weave_instances`, riding in `WaterUniforms` (now 512 B; the weave array is 16-aligned at offset 256). The instances are normalized, **position-agnostic analytical profiles** (location lives only in the instance: `world → local = rotate(world_xz − position, −orientation)/radius`), applied view-side in `water.wgsl` `vs_main` **after** the Gerstner sum (Q-cap untouched): `part`/`raise` as additive ±height, `freeze` as a **unified dual-channel** mask (suppresses waves + flattens normal + fragment material-state shift toward ice). Bounded to `±skirt_depth` (CPU intensity-clamp + shader net-clamp) and world-XZ sampled so it never re-exposes a LOD seam; **identity at zero instances** (runtime consumers set none). Editor-side test scaffolding (one part/raise/freeze) lives in `viewport/engine_adapter.rs::set_water_enabled` — the W.2c.3 plug-in seam for gameplay-emitted instances. Measured free on the 1660 Ti (~0.0005 ms/instance; full surface ~0.19–0.21 ms vs the 2.0 ms ceiling). **W.2c.3**: weave instances now arrive from a **binary-glue producer** (not just the editor scaffolding) via the new `Renderer::set_water_weave_instances(&[WeaveInstance])` delegation (renderer.rs; forwards to the private `water_renderer`). The producer (`examples/weaving_playground/src/weave_producer.rs::WaterWeaveProducer`) translates **positional gameplay `WeaveOp`s** — `LowerWater → part`, `RaisePlatform → raise`, `FreezeWater → freeze` (new `#[non_exhaustive]` variant, presentation-only truth) — into `WeaveInstance`s, reading only `op.a.xz` for position, with per-kind default intensity and an **age-driven ramp-in/hold/fade envelope** modulating `intensity` (so instantaneous ops get a visible duration; the `phase` field carries raw age as forward-provision — the W.2c.2 shader reads `intensity`, not `phase`, yet). The gameplay→render direction is **one-way**: the renderer takes only the render `WeaveInstance` type, so `astraweave-render` stays independent of the gameplay crates. **Coexist:** the ops' gameplay-truth behavior is unmodified (`LowerWater` still `clear_water()`s; `RaisePlatform` still Fortifies terrain); the render layer reads them in parallel. The render path (`water.rs`/`water.wgsl`) is byte-identical to W.2c.2 — the only render-crate change is the delegation. **F.4.2 (accent layer — machinery built, NOT yet visually live):** the splash/spray *accents* that garnish this surface exist but are **not yet drawn in a running frame**. `astraweave-fluids` now exposes `FluidRenderer::render_accents` (additive-billboard-only — the SSFR depth/smooth/shade chain is **not** invoked) and `FluidSystem::set_secondary_particles` (+ the `secondary_particle_count()` live-count fix — was hardcoded `65536`, which also stopped `fluids_demo` drawing 65k garbage billboards). A binary-glue `WaterAccentProducer` (`examples/weaving_playground/src/weave_accent_producer.rs`) — sibling to the weave producer, fed the same ops — translates weave triggers (`LowerWater → Part` silt spray / `RaisePlatform → Raise` lift-burst / `FreezeWater → Freeze` one-shot shimmer-then-suppress) into ballistically-aged `SecondaryParticle`s, style as tunable params (A1 procedural shape + B2 floaty motion in the producer; C3 per-kind tint + HDR glow in `secondary.wgsl`'s `info.y` LUT). Composition is **binary-orchestrated**: the binary depends on BOTH render and fluids, but `astraweave-render` and `astraweave-fluids` stay mutually independent (**no render↔fluids edge**; render byte-identical). **The live in-frame composite — uploading the accent set and injecting `render_accents` into the monolithic `render()` after the water pass — is F.4.3**; F.4.2's render path is GPU-proven in isolation (`gpu_render_accents_smoke`), but accents are not visible on the branch as of this commit. See `W2C1_RECON.md`, `W2C3_1_RECON.md`, `F4_0_RECON.md`, `F4_1_RECON.md`, `W2_DECISIONS.md` §B.1/§F, `W2A_EXECUTION_REPORT.md`, `W2B2_EXECUTION_REPORT.md`. |
...
| 17 | The gameplay→render weave seam is **one-way**: the renderer exposes only `Renderer::set_water_weave_instances(&[WeaveInstance])` (a render type), and the `WeaveOp → WeaveInstance` translation lives in **binary glue** (`weave_producer.rs`), so `astraweave-render` gains **no** dependency on `astraweave-gameplay`/`astraweave-weaving`. The ops' gameplay-truth behavior is unmodified (coexist) — `FreezeWater` is presentation-only (no walkable-ice/buoyancy this arc). | Yes | `renderer.rs` `set_water_weave_instances` (takes `crate::water::WeaveInstance`); `astraweave-render/Cargo.toml` has no gameplay edge; `weaving.rs::apply_weave_op` `FreezeWater` arm (budget+log only); W.2c.3 (`docs/campaigns/water-successor/W2C3_1_RECON.md`) |
| 18 | The weave-impact **accent** layer (F.4.2) reuses `astraweave-fluids`' secondary-billboard machinery and stays **crate-independent**: accents are produced by a binary-glue `WaterAccentProducer` and rendered by `FluidRenderer::render_accents` (additive-billboard-only — the SSFR surface chain is **never** invoked for accents), composited over the `WaterRenderer` surface by the **binary**. `astraweave-render` and `astraweave-fluids` have **no** dependency on each other (the binary depends on both). **F.4.2 is machinery-complete but NOT visually live** — the producer is wired (ingest+tick) and `render_accents` is GPU-proven in isolation, but the live in-frame upload + composite is **F.4.3**; accents are not drawn in a running frame yet. | Partial (machinery only; live composite is F.4.3) | `astraweave-fluids/src/renderer.rs` `render_accents` (no depth/smooth/shade); `set_secondary_particles` + `secondary_particle_count` live-count (`lib.rs`, was hardcoded 65536); `examples/weaving_playground/src/weave_accent_producer.rs`; neither `astraweave-render/Cargo.toml` nor `astraweave-fluids/Cargo.toml` references the other; `gpu_render_accents_smoke`; F.4.2 (`docs/campaigns/water-successor/F4_0_RECON.md`, `F4_1_RECON.md`) |
