...
use self::material::MaterialGpu;
use self::material::MaterialLibrary;
...
// Minimal value-noise based "Perlin" helper compatible with Perlin::new(seed).get([x,y,z])
struct Perlin {
    seed: u32,
}
...
impl Perlin {
    ...
    fn hash3(x: i32, y: i32, z: i32, seed: u32) -> u32 {
        let mut h = x as u32;
        h = h.wrapping_mul(0x27d4_eb2d) ^ (y as u32).rotate_left(13);
        h ^= (z as u32).wrapping_mul(0x85eb_ca6b).rotate_left(7);
    ...
    fn noise3(&self, x: f64, y: f64, z: f64) -> f64 {
        ...
        // Corner random values in [0,1]
        let val = |dx: i32, dy: i32, dz: i32| -> f64 {
            let h = Self::hash3(x0 + dx, y0 + dy, z0 + dz, self.seed);
            // Map u32 to [0,1]
    ...
    pub fn get(&self, sample: [f64; 3]) -> f64 {
        self.noise3(sample[0], sample[1], sample[2])
    }
}
...
fn generate_tree_impostor_mesh() -> MeshData {
    let mut builder = MeshBuilder::new();
    // Crossed billboards approximating a leafy tree crown
    add_billboard_cross(
        &mut builder,
...
fn generate_skybox_mesh() -> MeshData {
    // Build directly from helper skybox vertices
...
fn generate_tree_mesh(seed: u32) -> MeshData {
    let mut rng = SimpleRng::new(seed as u64 + 0x7e_7e);
    let mut builder = MeshBuilder::new();

    // Trunk
    let height = rng.range_f32(2.5, 3.6);
    let trunk_r0 = rng.range_f32(0.12, 0.18);
    let trunk_r1 = trunk_r0 * rng.range_f32(0.65, 0.85);
    let tilt = Vec3::new(rng.range_f32(-0.2, 0.2), height, rng.range_f32(-0.2, 0.2));
    ...
    // Canopy: a few overlapping spheres
    let canopy_center = Vec3::new(tilt.x * 0.2, height, tilt.z * 0.2);
    for i in 0..3 {
        let ang = i as f32 * (TAU / 3.0);
        let offs = Vec3::new(ang.cos() * 0.4, rng.range_f32(-0.1, 0.3), ang.sin() * 0.4);
        let r = rng.range_f32(0.7, 1.1);
        add_latlong_sphere(
            &mut builder,
            canopy_center + offs,
            r,
...
fn generate_house_mesh(seed: u32) -> MeshData {
    let mut rng = SimpleRng::new(seed as u64 + 0x51_50);
    let mut builder = MeshBuilder::new();

    // Base building box
    let w = rng.range_f32(1.6, 2.4);
    let d = rng.range_f32(1.4, 2.0);
    let h = rng.range_f32(1.2, 1.8);
    add_box(
        &mut builder,
    ...
    // Simple gable roof approximated by a taller top box
    let roof_h = rng.range_f32(0.4, 0.8);
    add_box(
        &mut builder,
        Vec3::new(-w * 0.5, h, -d * 0.5),
        Vec3::new(w * 0.5, h + roof_h, d * 0.5),
        Vec2::new(1.2, 0.8),
    );
...
fn generate_character_mesh(seed: u32) -> MeshData {
    ...
    let pelvis_half = Vec3::new(
        stance_width * 0.5 + thigh_radius * 0.6,
        thigh_radius * 0.8,
        0.18,
    );
    add_box(
        ...
        Vec3::new(
            -pelvis_half.x,
            pelvis_height - pelvis_half.y,
            -pelvis_half.z,
        ),
        Vec3::new(
            pelvis_half.x,
            pelvis_height + pelvis_half.y * 0.8,
            pelvis_half.z,
        ),
        Vec2::new(0.8, 0.6),
    );
    ...
    if rng.range_u32(0, 100) < 55 {
        ...
        add_billboard_cross(
            ...
            Vec3::new(
                0.0,
                torso_top + neck_height + head_radius * 0.65,
                head_radius * 0.9,
            ),
    ...
    if rng.range_u32(0, 100) < 30 {
        add_box(
            &mut builder,
            Vec3::new(
                -torso_radius_bottom * 0.6,
                pelvis_height + 0.1,
                -torso_radius_bottom * 1.4,
            ),
            Vec3::new(
                torso_radius_bottom * 0.6,
                pelvis_height + torso_height * 0.8,
                -torso_radius_bottom * 0.6,
            ),
            Vec2::new(0.8, 1.4),
        );
...
fn pattern_noise(seed: u32, x: u32, y: u32) -> u8 {
    ...
    let mut v = x.wrapping_mul(374_761_393).rotate_left(5)
...
fn find_texture_entry_id_for_path(manager: &TextureManager, texture_path: &str) -> Option<String> {
    ...
    manager.get_all_textures().iter().find_map(|(id, entry)| {
...
fn load_material_layer_images_fallback(
    def: &MaterialLayerDefinition,
    width: u32,
    height: u32,
) -> (RgbaImage, RgbaImage, RgbaImage) {
...
fn load_material_images_for_layer(
    ...
    if let Some(manager) = texture_manager {
        if let Some(images) = load_material_images_for_layer_with_manager(
            def, material, width, height, manager, asset_root,
        ) {
            return images;
        }
    }
...
// Resolve a texture path relative to an asset root. Supports absolute and a few common subfolders.
fn resolve_texture_path(asset_root: &Path, rel: &str) -> Option<PathBuf> {
    let p = Path::new(rel);
    ...
    let candidates = [
        asset_root.join(rel),
        asset_root.join("textures").join(rel),
        asset_root.join("materials").join(rel),
    ];
    for c in candidates {
        if c.exists() {
            return Some(c);
...
// Try to infer a normal map path from an albedo path by suffix conventions.
fn find_normal_map(albedo_path: &Path) -> Option<PathBuf> {
    let parent = albedo_path.parent()?;
    let stem = albedo_path.file_stem()?.to_string_lossy();
    let candidates = [
        format!("{}_n.png", stem),
        format!("{}_normal.png", stem),
        format!("{}-normal.png", stem),
        format!("{}_n.jpg", stem),
        format!("{}_normal.jpg", stem),
    ];
    for c in candidates {
        let p = parent.join(c);
        if p.exists() {
...
pub(crate) fn load_biome_toml(biome_name: &str) -> Option<(BiomeToml, std::path::PathBuf)> {
    ...
    let base = asset_dir();
    log::debug!("[biome-dbg] asset_dir() -> {}", base.display());
    // Print candidate paths we try (for visibility in unit tests)
    let cp = candidate_paths();
    for p in cp.iter() {
        log::debug!("[biome-dbg] candidate_paths item: {}", p.display());
    }
    let candidates = [
        base.join("textures")
            .join(biome_name)
    ...
    for path in candidates {
        log::debug!("[biome-dbg] trying candidate path: {}", path.display());
        if path.exists() {
            log::debug!("[biome-dbg] found via candidate: {}", path.display());
            if let Ok(txt) = std::fs::read_to_string(&path) {
                match toml::from_str::<BiomeToml>(&txt) {
                    ...
                    Err(err) => {
                        log::warn!(
                            "[biome-dbg] toml parse error for {}: {}",
                            path.display(),
                            err
                        );
                        // Try to be tolerant of markdown-style fences (```toml ... ```)
                        let stripped = txt
                            .trim()
                            ...
                            .to_string();
                        if stripped != txt {
                            if let Ok(doc) = toml::from_str::<BiomeToml>(&stripped) {
                                log::debug!(
                                    "[biome-dbg] parsed after stripping code fences for {}",
                                    path.display()
                                );
                                let dir = path
                                    .parent()
                                ...
                                log::warn!(
                                    "[biome-dbg] still failed to parse after strip: {}",
                                    path.display()
                                );
                            }
                        }
    ...
    // Use relative patterns that will be joined against asset_dir() so avoid repeating "assets/".
    let fallback_rels = [
        format!("textures/{}/materials.toml", biome_name),
        format!("{}/materials.toml", biome_name), // assets/<biome>/materials.toml
        // test harness places synthetic biomes under assets/tests/<biome>
        format!("tests/{}/materials.toml", biome_name),
        // keep an explicit example-local override if present
        format!(
            "examples/unified_showcase/assets/{}/materials.toml",
            biome_name
        ),
    ];
    for rel in fallback_rels.iter() {
        // Try a manifest-relative candidate first for clarity
        let manifest_candidate = manifest_dir.join(rel);
        log::debug!(
            "[biome-dbg] trying manifest-relative fallback: {}",
            manifest_candidate.display()
        );
        if manifest_candidate.exists() {
            log::debug!(
                "[biome-dbg] found via manifest-relative: {}",
                manifest_candidate.display()
            );
            if let Ok(txt) = std::fs::read_to_string(&manifest_candidate) {
                if let Ok(doc) = toml::from_str::<BiomeToml>(&txt) {
                    let dir = manifest_candidate
                        .parent()
                        .unwrap_or_else(|| std::path::Path::new("."))
                        .to_path_buf();
                    return Some((doc, dir));
        ...
        // Next try resolve_asset_path which uses the candidate_paths/asset_dir logic.
        let resolved = resolve_asset_path(rel);
        log::debug!(
            "[biome-dbg] resolve_asset_path('{}') -> {}",
            rel,
            resolved.display()
        );
        if resolved.exists() {
            log::debug!(
                "[biome-dbg] found via resolve_asset_path: {}",
                resolved.display()
            );
            if let Ok(txt) = std::fs::read_to_string(&resolved) {
                if let Ok(doc) = toml::from_str::<BiomeToml>(&txt) {
                    let dir = resolved
                        .parent()
                        .unwrap_or_else(|| std::path::Path::new("."))
...
/// Initialize logging for test runs. Idempotent.
pub(crate) fn init_logging_for_tests() {
    use std::sync::Once;
    static START: Once = Once::new();
    START.call_once(|| {
        // Configure env_logger with default filter if RUST_LOG is not set.
...
pub(crate) fn pack_mra_from_planes(
    metal: &[u8],
    rough: &[u8],
    ao: &[u8],
    width: u32,
    ...
    assert_eq!(
        ...
        "metal plane must be RGBA bytes"
    );
    assert_eq!(
        rough.len(),
        (width * height * 4) as usize,
        "rough plane must be RGBA bytes"
    );
    assert_eq!(
        ao.len(),
        (width * height * 4) as usize,
        "ao plane must be RGBA bytes"
    );
...
fn load_layer_from_biome_spec(
    def: &MaterialLayerDefinition,
    spec: &BiomeLayer,
    base_dir: &std::path::Path,
    width: u32,
    height: u32,
) -> (RgbaImage, RgbaImage, RgbaImage, bool, bool, bool) {
    ...
    let albedo = if let Some(path) = &spec.albedo {
        if let Some(img) = try_load_image_relative(base_dir, path, width, height) {
            ...
            log::warn!(
                "[textures] missing albedo for {}/{} → substituting fallback",
                base_dir.display(),
                def.key
            );
            def.fallback_albedo(width, height)
    ...
    let normal = if let Some(path) = &spec.normal {
        if let Some(img) = try_load_image_relative(base_dir, path, width, height) {
            ...
            log::warn!(
                "[textures] missing normal for {}/{} → substituting neutral",
                base_dir.display(),
                def.key
            );
            create_flat_normal_image(width, height)
        ...
        if let Some(img) = try_load_image_relative(base_dir, path, width, height) {
            ...
            if let (Some(m), Some(r), Some(ao)) = (&spec.metallic, &spec.roughness, &spec.ao) {
                if let Some(packed) = pack_mra_from_separate(base_dir, m, r, ao, width, height) {
                    ...
                    log::warn!(
                        "[textures] missing MRA for {}/{} → substituting neutral",
                        base_dir.display(),
                        def.key
                    );
...
fn create_material_array_textures(
    ...
    let _ = (device, queue, texture_manager, biome_hint);
    Err(anyhow::anyhow!(
        "legacy create_material_array_textures is disabled; use MaterialIntegrator::load and bind its arrays"
    ))
}
...
#[cfg(test)]
pub(crate) fn stats_for_biome_loading(
    biome_name: &str,
    width: u32,
    height: u32,
    ...
    for def in MATERIAL_LAYERS.iter() {
        if let Some(spec) = find_biome_layer(&doc, def.key) {
            let (_a, _n, _m, a_ok, n_ok, m_ok) =
                load_layer_from_biome_spec(def, spec, &base_dir, width, height);
            if a_ok {
                albedo_loaded += 1;
            }
            if n_ok {
...
struct RenderStuff {
    ...
    pipeline_full: wgpu::RenderPipeline,
    ...
    // Shared materials manager for authored packs (arrays.toml/materials.toml)
    material_integrator: MaterialIntegrator,
...
fn add_latlong_sphere(
    ...
    for ring_idx in 0..rings.len().saturating_sub(1) {
        ...
        for lon in 0..lon_segments {
            let next = (lon + 1) % lon_segments;
            builder.add_quad(current[lon], current[next], next_ring[next], next_ring[lon]);
        }
    }
}
...
impl UiState {
    ...
    fn refresh_info_text(&mut self, character_count: usize) {
        let profile = self
            .active_lod_settings()
            .for_category(self.active_lod_category);
...
fn load_texture_from_bytes_with_format(
    ...
    log::debug!("Loading texture '{}' from {} bytes", label, bytes.len());
    ...
    log::debug!("Successfully created texture resources for '{}'", label);
...
fn vs(@builtin(vertex_index) vi: u32) -> VsOut {
    var out: VsOut;
    // fullscreen covering triangle -- avoid indexing a literal array with a non-constant
    var xy: vec2<f32>;
    if (vi == 0u) {
        xy = vec2<f32>(-1.0, -1.0);
    } else if (vi == 1u) {
...
fn fs(in: VsOut) -> @location(0) vec4<f32> {
    // Simple linear sample from previous mip
...
fn load_texture_from_file(
    ...
    log::debug!("Attempting to load texture from file: {}", path.display());
    if !path.exists() {
        log::warn!("ERROR: Texture file does not exist: {}", path.display());
    ...
    let bytes = fs::read(path).map_err(|e| {
        log::warn!("Failed to read texture file: {}", e);
    ...
    let format = if is_normal || is_mra {
        ...
        log::debug!("Inferred sRGB format for texture: {}", path.display());
...
// Materials overrides and reloads are handled via authored TOML packs and MaterialIntegrator
...
fn switch_biome(
    ...
    // Load or fetch material pack for this biome and rebind arrays
    if let Ok(rt) = pollster::block_on(render.material_integrator.load(
        &render.device,
        &render.queue,
        biome_name,
    )) {
        // Rebuild ground/material bind group using the integrator's layout
        let new_bg = render.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("materials-pack (switch-biome)"),
            layout: &render.texture_bind_group_layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&rt.gpu.albedo),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::Sampler(&rt.gpu.sampler_albedo),
                },
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: wgpu::BindingResource::TextureView(&rt.gpu.normal),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
                    resource: wgpu::BindingResource::Sampler(&rt.gpu.sampler_linear),
                },
                wgpu::BindGroupEntry {
                    binding: 4,
        ...
        render.material_integrator.set_current(biome_name);
    } else {
        eprintln!(
            "[materials] Failed to load biome '{}'; keeping previous material arrays.",
            biome_name
        );
    }
...
mod tests {
    ...
    fn loads_grassland_biome_toml() {
        let (doc, _base) =
            load_biome_toml("grassland").expect("grassland materials.toml should parse");
...
async fn run() -> Result<()> {
    ...
    // Window will be created in ApplicationHandler; let window = std::sync::Arc::new(event_loop.create_window(window_attributes)?);
                                                ...
                                                match pollster::block_on(render.material_integrator.reload(&render.device, &render.queue, &biome)) {
                                                    Ok(rt) => {
                                                        let new_bg = render.device.create_bind_group(&wgpu::BindGroupDescriptor {
                                                            ...
                                                            layout: &render.texture_bind_group_layout,
                                                            entries: &[
                                                                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&rt.gpu.albedo) },
                                                                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&rt.gpu.sampler_albedo) },
                                                        ...
                                                        render.ground_bind_group = new_bg;
                                                        render.material_integrator.set_current(&biome);
                                                        println!("[materials] Hot reloaded biome '{}'.", biome);
                                                    }
                                                    Err(err) => {
                                                        eprintln!("[materials] Hot reload failed for '{}': {}", biome, err);
                                                    }
                                                }
                                            } else {
                                                // Throttle IBL rebake to avoid spamming GPU work
                    ...
                    WindowEvent::RedrawRequested => {
                        ...
                        // Shadow pass
                        {
                            ...
                            for batch in &instance_batches {
                                if matches!(batch.mesh_key.category, MeshCategory::Skybox) { continue; }
                                render.queue.write_buffer(&render.instance_vb, 0, bytemuck::cast_slice(&batch.instances));
                                // Prefer GPU mesh override from registry (position-only buffer for shadow pass)
                                if let Some(&h) = render.mesh_overrides.get(&batch.mesh_key) {
                                    if let Some(gpu) = render.mesh_registry.get_gpu(h) {
                                        sp.set_vertex_buffer(0, gpu.vertex_pos.slice(..));
                                        sp.set_vertex_buffer(1, render.instance_vb.slice(..));
                                        sp.set_index_buffer(gpu.index.slice(..), wgpu::IndexFormat::Uint32);
                                        if !batch.instances.is_empty() { sp.draw_indexed(0..gpu.index_count, 0, 0..batch.instances.len() as u32); }
                                        continue;
                                    }
                                }
                            ...
                            for batch in &instance_batches {
                                ...
                                if let Some(&h) = render.mesh_overrides.get(&batch.mesh_key) {
                                    if let Some(gpu) = render.mesh_registry.get_gpu(h) {
                                        ...
                                        if !batch.instances.is_empty() { rp.draw_indexed(0..gpu.index_count, 0, 0..batch.instances.len() as u32); }
                                        continue;
                                    }
                                }
                                // Fallback to local procedural mesh
...
fn create_all_meshes(device: &wgpu::Device) -> std::collections::HashMap<MeshKey, Mesh> {
    ...
    meshes.insert(
        MeshKey::new(MeshCategory::Character, CHARACTER_IMPOSTOR_VARIANT),
...
@fragment fn fs(in: VsOut) -> @location(0) vec4<f32> {
    ...
    // Sample array layer using 2D coords + i32 layer index
    let colr = textureSample(albedo_layers, albedo_sampler, frac, layer_idx).rgb;
...
"#;
    ...
    let texture_bind_group_layout = material_integrator.bind_group_layout_owned(&device);
    ...
    // Provide a default material uniform so group(4) is always valid
    let default_material_gpu: MaterialGpu = (&Material::default()).into();
    let default_material_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("default-material-ub"),
        contents: bytemuck::bytes_of(&default_material_gpu),
        usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
    });
    ...
    let ground_bind_group = {
        ...
        device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&pack_rt.gpu.albedo),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::Sampler(&pack_rt.gpu.sampler_albedo),
                },
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: wgpu::BindingResource::TextureView(&pack_rt.gpu.normal),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
                    resource: wgpu::BindingResource::Sampler(&pack_rt.gpu.sampler_linear),
                },
                wgpu::BindGroupEntry {
                    binding: 4,
    ...
    material_integrator.set_current(current_biome);

    // Developer UX: startup summaries (materials, IBL, mesh overrides) moved below after resources init
    ...
    if let Some(ref mut mgr) = ibl2_manager {
        // Prefer HDR equirectangular if present: assets/ibl/<biome>.hdr
        let hdr_path = asset_dir()
            .join("ibl")
            .join(format!("{}.hdr", current_biome));
        if hdr_path.exists() {
            mgr.mode = SkyMode::HdrPath {
                biome: current_biome.to_string(),
                path: hdr_path.to_string_lossy().into_owned(),
            };
            println!(
                "[ibl] Using HDR equirectangular for biome '{}': {}",
                current_biome,
                hdr_path.display()
            );
        } else {
            println!(
                "[ibl] No HDR found for biome '{}'; using procedural sky.",
                current_biome
            );
        }
    ...
    // Discover optional glTF models under assets/models and register GPU overrides per category
    let models_root = asset_dir().join("models");
    let mut mesh_overrides_scan: HashMap<MeshKey, MeshHandle> = HashMap::new();
    if models_root.exists() {
        let mut merged = load_gltf_category_variants(
            &device,
            &queue,
            &mut mesh_registry,
            &models_root,
        ...
        mesh_overrides_scan.extend(merged.drain());
        let mut merged = load_gltf_category_variants(
            &device,
            &queue,
            &mut mesh_registry,
        ...
        mesh_overrides_scan.extend(merged.drain());
        if mesh_overrides_scan.is_empty() {
            println!("[models] assets/models present but no .gltf/.glb found; continuing with procedural meshes");
        } else {
            println!(
                "[models] Loaded {} glTF overrides from {}",
                mesh_overrides_scan.len(),
                models_root.display()
            );
        }
    } else {
        println!("[models] assets/models not found; continuing with procedural meshes");
    }

    // Developer UX: startup summaries (materials, IBL, mesh overrides)
    if let Some(rt) = material_integrator.current() {
        println!("{}", rt.stats.concise_summary());
    }
    if let Some(ref mgr) = ibl2_manager {
        println!("[ibl] mode={:?} quality=Medium", mgr.mode);
    }
    if mesh_overrides_scan.is_empty() {
        println!("[models] No glTF overrides; using procedural meshes");
    } else {
        println!("[models] {} overrides active", mesh_overrides_scan.len());
    }
    ...
    let pipeline_full = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: Some("pipeline-full-vertex"),
        layout: Some(&pipeline_layout),
        vertex: wgpu::VertexState {
            module: &shader_module,
            ...
            buffers: &[
                ...
                // Instance buffer starts at location 4 in WGSL (m0..m3,color,mesh_type)
                wgpu::VertexBufferLayout {
                    array_stride: std::mem::size_of::<InstanceRaw>() as u64,
                    step_mode: wgpu::VertexStepMode::Instance,
                    attributes: &[
    ...
    Ok(RenderStuff {
        ...
        default_material_buffer: default_material_buffer,
        default_material_bind_group: default_material_bg,
        ...
        ibl2_manager,
        mesh_registry,
        mesh_overrides: mesh_overrides_scan,
    })
}
...
// ---------------- models (glTF) intake and overrides ----------------

fn scan_models_dir(base: &Path) -> Vec<PathBuf> {
    let mut out = Vec::new();
    for entry in fs::read_dir(base).unwrap_or_else(|_| fs::read_dir(".").unwrap()) {
        if let Ok(e) = entry {
...
fn load_gltf_category_variants(
    device: &wgpu::Device,
    queue: &wgpu::Queue,
    ...
    max_variants: u32,
    key_prefix: &str,
) -> HashMap<MeshKey, MeshHandle> {
    let mut overrides = HashMap::new();
    let dir = root.join(match cat {
        MeshCategory::Tree => "trees",
        MeshCategory::Rock => "rocks",
        MeshCategory::House => "houses",
        MeshCategory::Character => "characters",
        _ => "",
    });
    if !dir.exists() {
        return overrides;
    }
    let files = scan_models_dir(&dir);
    if files.is_empty() {
        return overrides;
    }
    let opts = astraweave_render::mesh_gltf::GltfOptions::default();
    ...
    for file in files {
        match astraweave_render::mesh_gltf::load_gltf(&file, &opts) {
            Ok(meshes) => {
                for (i, cpu) in meshes.iter().enumerate() {
                    let v = variant_idx % max_variants;
                    let key = RenderMeshKey(format!(
                        "{}:{}:{}:{}",
                        key_prefix,
                        match cat {
                            MeshCategory::Tree => "tree",
                            MeshCategory::Rock => "rock",
                            MeshCategory::House => "house",
                            MeshCategory::Character => "char",
                            _ => "misc",
                        },
                        file.file_stem().and_then(|s| s.to_str()).unwrap_or("mesh"),
                        i
                    ));
                    if let Ok(h) = reg.fetch_or_upload(device, queue, key, cpu) {
                        overrides.insert(MeshKey::new(cat, v), h);
                    }
                    variant_idx = variant_idx.wrapping_add(1);
                }
            }
            Err(err) => {
                eprintln!("[models] Failed to load {}: {}", file.display(), err);
            }
        }
    }
    overrides
...
fn sync_instances_from_physics(
    ...
    for (_h, body) in p.bodies.iter() {
        ...
        let lod_flags = if lod.variant_override.is_some() {
            LOD_FLAG_IMPOSTOR
        } else {
            0
        };
